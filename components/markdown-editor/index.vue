...
export default {
  ...
  data() {
    return {
      ref: `markdown-edirot-${Math.floor(new Date().getTime() + Math.random() * 10000).toFixed(0)}`,
      toolbars: {
        bold: true, // 粗体
        italic: true, // 斜体
        header: true, // 标题
  ...
  mounted() {
    // 内容发生变化时保存
    if (this.autoSave) {
      this.handleChange = _.throttle(this.handleSave, 1000)
    }
  },
  methods: {
    /* 添加图片或掉事件 */
    ...
    handleSave(value) {
      // 有效期3天
      Storage.set(this.autoSaveKey, value, 259200000)
    },
    // 恢复自动保存的数据
    recover() {
      const data = Storage.get(this.autoSaveKey) || ''
      this._value = data
      return data
    },
    // 清除所有自动保存的数据
    remove() {
      Storage.remove(this.autoSaveKey)
    }
  }
}
</script>
